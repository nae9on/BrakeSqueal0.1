...
2. previous_shifts (python list for the previous shift points already calculated)
3. previous_radius (corresponding radius of the previous shift points)

Output
1. next_shift (next shift point in the target region)
...
def next_shift(target,previous_shifts = [],previous_radius = []):
...
def calculate_area_fraction(target,previous_shifts,previous_radius):
	...
	for i in range(0,pool):
		  ...
		  for j in range(0,len(previous_shifts)):
		     ...
		     if r<previous_radius[j]:
		      inn = inn+1
	              break
	fraction_increment = inn/pool
	return fraction_increment 			

def draw_circles(target,next_shift,next_radius):
