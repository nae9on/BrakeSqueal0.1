...
####################################################################
print '\n\n----------------------------------------------------------------------------------------'
laTraditional, evecTraditional = traditionalProjection.Obtain_eigs(obj,150)
XTraditional = numpy.concatenate((evecTraditional.real,evecTraditional.imag), axis=1)
...
for setItr in range(0,len(X)):
	...
	if setItr == 0:
	 print '\n\n Traditional Projection with size of measurment matrix = '+str(X[setItr].shape)
	else:
...
plt.subplot(221)
plt.plot(xDim, yDeltaList[0], '-ro', label='Traditional')
...
plt.subplot(222)
plt.plot(xDim, yTheta1List[0], '-ro', label='Traditional')
...
brake.save(obj.output_path+'traditionalVsPOD'+str(obj.omegaTest/(2*math.pi)), ext="png", close=False, verbose=True)
plt.show()
